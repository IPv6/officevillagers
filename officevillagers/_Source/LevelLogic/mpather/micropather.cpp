...
void MicroPather::GoalReached( PathNode* node, void* start, void* end, irr::core::array< void* > *path )
{
	path->clear();
...
NodeCost* MicroPather::GetNodeNeighbors( PathNode* node, irr::core::array< StateCost >* neighbor, irr::core::array< NodeCost >* neighborNode )
{
	NodeCost* nodeCost = 0;
		 ...
		 || node->numAdjacent > PathNode::MAX_CACHE )
	{
		...
		if ( node->numAdjacent <= PathNode::MAX_CACHE ) 
		{
			...
			neighborNode->set_used( neighbor->size() );
...
void MicroPather::StatesInPool( irr::core::array< void* >* stateVec )
{
 	stateVec->clear();
...
int MicroPather::Solve( void* startNode, void* endNode, irr::core::array< void* >* path, float* cost, int iMaxDepth )
{
	#ifdef DEBUG_PATH
	printf( "Path: " );
...
//		ReuseAll();
	...
	neighborVec.set_used(0);
	neighborNodeVec.set_used(0);
	int iDepth=0;
	while ( !open.Empty() )
	{
		if(iMaxDepth && iDepth>iMaxDepth){
			break;
		}
		iDepth++;
