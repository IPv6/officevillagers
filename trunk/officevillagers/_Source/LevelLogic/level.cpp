	...
	if(!bAll){
		if(aHints.size()>0){
			...
			if(iPriority>=0 && nht.iPriority>iPriority){
				// Не можем мы так снять его... приоритетом не вышли
				return;
			}
			aHints.erase(0);
		}
...
void CLevel::AddHint(CString sHudText, f32 fTime, int iPriority, DWORD dwColor, BOOL bCloseBaton)
{
	...
	if(iPriority==0){
		// Для нулевого приоритета - не стекируем, затираем только если счас тоже с 0ым
		if(aHints.size()>0){
			CSingleHint& nht=aHints[0];
			if(nht.iPriority>0){
				return;
	...
	if(aHints.size()==0 || aHints[0].iPriority>hnt.iPriority){
		...
		// В начало, пришел более важный или такойже
		aHints.insert(hnt,0);
	}
	ActualizeHint();
}
